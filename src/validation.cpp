...
#if defined(NDEBUG)
# error "ROCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "ROCoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ROC additional fees, %d delta bytes\n",
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip()->GetBlockTime() < (GetTime() - nMaxTipAge))
        return false; //stuff for beginning
