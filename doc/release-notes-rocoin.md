ROCoin Core version 0.18.1 is now available from:

  <https://download.rocoin.org/rocoin-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/rocoin-dev>
...
`/Applications/ROCoin-Qt` (on Mac) or `rocoind`/`rocoin-qt` (on
Linux).
...
ROCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use ROCoin Core on unsupported systems.

ROCoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, ROCoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/rocoin-project/rocoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@rocoin.org) the ROCoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 ROC),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in ROC per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/rocoin-project/rocoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/rocoin-project/rocoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/rocoin-project/rocoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/rocoin-project/rocoin/blob/master/doc/rocoin-conf.md)
  about the `rocoin.conf` file describes how to use it to configure
  ROCoin Core.

- A new document introduces ROCoin Core's BIP174 [Partially-Signed
  ROCoin Transactions
  (PSBT)](https://github.com/rocoin-project/rocoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/rocoin-project/rocoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent ROCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  ROCoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/rocoin-project/rocoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/rocoin-project/rocoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when ROCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of ROCoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/rocoin-project/rocoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/rocoin-project/rocoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart rocoind with the `-deprecatedrpc=generate`
...
- A new `rocoin-wallet` tool is now distributed alongside ROCoin
...
This section describes planned changes to ROCoin Core that may affect
other ROCoin software and services.

- Since version 0.16.0, ROCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with ROCoin Core 0.20 (expected about
  a year after 0.18), ROCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the ROCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in ROCoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any ROCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, ROCoin Core
...
- A sub-project of ROCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with ROCoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to ROCoin Core's own implementation, although entropy
  gathered by ROCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves ROCoin Core a
...
- On macOS, ROCoin Core now opts out of application CPU throttling
